...
void Buzzer_Init(void)
{
	...
	SET_PIN_DIRECTION(BuzzerPort, BuzzerPin, 1);
	WRITE_PIN_VALUE(BuzzerPort, BuzzerPin, 0);//Set the Buzzer's Pin as Output
}

void Buzzer_ON(void)
{
	WRITE_PIN_VALUE(BuzzerPort, BuzzerPin, 0);		//Set the Buzzer High
}

void Buzzer_OFF(void)
{
	WRITE_PIN_VALUE(BuzzerPort, BuzzerPin, 1);		//Set the Buzzer Low
}

void Buzzer_BUZZ(void)			//Buzzer Buzzes Once
{
		Buzzer_ON();
		Delay_OneMilliSec(80);
		Buzzer_OFF();
		Delay_OneMilliSec(80);
}
